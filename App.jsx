...
// Imágenes de ejemplo para el carrusel (puedes reemplazar las URLs por imágenes locales si tienes)
...
function App() {
  const [view, setView] = useState("inicio"); // Cambia el inicio a "inicio"
  ...
  // NUEVO: Estado para usuario logueado
  const [usuario, setUsuario] = useState({ nombre: "", cargo: "" });

  // Proyectos de ejemplo (ahora con estado)
  ...
  // Estado para edición de proyecto
  ...
  // NUEVO: Estados para login y registro
  ...
  // Carrusel automático
  // Puedes comentar esto si no quieres que cambie solo
  // useEffect(() => {
  ...
  const handleRegistro = (e) => {
    e.preventDefault();
    // Agrega el nuevo empleado al sistema (solo frontend)
    setEmpleados(emps => [
      ...emps,
      {
